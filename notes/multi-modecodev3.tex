...
\begin{eqnarray}
\frac{D\f^a}{\ud t} + 3H \dot\f^a + \gamma^{ab} V_{; c} = 0 ~. 
\end{eqnarray}
where we have $V_{;c} \equiv D V / d \f^c = \pd V/\pd \phi^c$, and
\[
...
$T^a \equiv \dot{\f}^a/\dot{\f}_0$ and $\epsilon=-H_{,\alpha}/H$.
...
Since the mass matrix Eq.~\eqref{eqn:massmatrix} is not necessarily diagonal, the perturbation equations~\eqref{eqn:ptbmodeeqn} mix the annihilation operators for all of the fields \cite{Salopek:1988qh}. We therefore need to expand each perturbation mode $u_I(\bk)$ using $N$ harmonic oscillators $a_J(\bk)$,
\begin{equation}
u_I(\bk, \alpha) = \sum_J \psi_{IJ}(\bk, \alpha) a_J(\bk) ~,
\end{equation}
with
\begin{equation}
[ a_J(\bk), a^\dagger_I(\bk')] = (2\pi)^2 \delta_{IJ} \delta^{(3)}(\bk - \bk')
\end{equation}
...
\begin{equation}
{\color{blue}
P_Q^{IJ} (k) = \frac{k^3}{2\pi^2} \sum_L \frac{\psi_{IL}\psi^*_{JL}}{a^2}
}
\end{equation}
