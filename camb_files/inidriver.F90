...
!MODIFIED P(K)
	...
	kSrcSampleBoost	= Ini_Read_Double('k_src_sample_boost',1.d0)
	kIntSampleBoost	= Ini_Read_Double('k_int_sample_boost',1.d0)
	DoClsInterpol	= Ini_Read_Logical('do_cls_interpol',.true.)
	ComputeEveryl	= Ini_Read_Logical('compute_every_l',.false.)

       if ((ComputeEveryl).and.((kSrcSampleBoost>1.d0).or. &
	  (kIntSampleBoost>1.d0))) then
		write(*,*) ' ************************* '
		write(*,*) 'compute_every_l option only allowed if k_src_sample_boost=k_int_sample_boost=1.'
		write(*,*) 'Setting compute_every_l = F.'
		write(*,*) ' ************************* '
		ComputeEveryl = .false.
                DoClsInterpol = .true.
       end if
       ...
       if (FeedbackLevel > 0) then
          write (*,*)'do_cls_interpol is',DoClsInterpol
          write (*,*)'compute_every_l is',ComputeEveryl
