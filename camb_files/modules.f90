...
!      integer, parameter :: lmax_arr = l0max !!!100+l0max/7
      integer, parameter :: lmax_arr = 100+l0max
      real(dl) :: kSrcSampleBoost=1._dl
      real(dl) :: kIntSampleBoost=1._dl
      logical :: DoClsInterpol=.true.
      logical :: ComputeEveryl=.false.
!END MODIFIED P(K)
...
!MODIFIED P(K)       
        if (ComputeEveryl.and.(FeedbackLevel>0)) then
           write(*,*) 'Warning: computing every l'           
           if (max_l.gt.lmax_arr) then
              write(*,*)'lmax_arr maxl',lmax_arr,max_l
              stop
           endif
           lind=0
           do lvar=2, max_l
